...
<body>
	...
	<div
		...
		<table style="width: 100%; align-content: center;" id="mtable">
			...
			<c:forEach items="${MoviesList}" var="i">
				<tbody>
					<tr>
						<td>${i.m_name}</td>
						<td>${i.m_oneline}</td>
			...
			<tr>
				<td colspan="10"><c:if test="${pageVO.prev}">
						<span><a
							href="MoviesManagerMent?page=${pageVO.startPage -1}">[이전페이지그룹]</a></span>
					</c:if> <!--  page그룹 정보출력  변수명  startPage   endPage--> <c:forEach
						begin="${pageVO.startPage }" end="${pageVO.endPage }" var="idx">
						<span><a href="MoviesManagerMent?page=${idx}">${idx}</a></span>
					</c:forEach> <c:if test="${pageVO.next}">
						<span><a href="MoviesManagerMent?page=${pageVO.endPage +1}">[다음페이지그룹]</a></span>
					</c:if></td>
			</tr>
		</table>
